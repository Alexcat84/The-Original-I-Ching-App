...
2. **Guard de CI** (`.github/workflows/ci.yml` → job `resolution-guard`, **bloqueante** desde 2026-07-14 tras varias corridas verdes): con `npm@10.9.2` regenera el lockfile desde cero (sin flags) y **falla el PR** si (a) hay ERESOLVE o (b) el web no resuelve `18.x` o mobile no `19.0.0`; además **reporta** la versión hoisteada al root (el WARN de no-determinismo `root != web` de §2.b sigue siendo aviso **no** bloqueante dentro del script). Convierte cualquier ruptura de resolución en barrera de merge. **Trade-off:** el paso de regeneración toca el registry de npm en cada PR; un hipo de red podría dar rojo falso — si eso pasa, la respuesta correcta es **añadir un reintento al paso de regeneración, no** volver a `continue-on-error`.
