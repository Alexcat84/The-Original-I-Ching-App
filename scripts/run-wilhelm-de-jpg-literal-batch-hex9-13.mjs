#!/usr/bin/env node
/**
 * QA code: AU-FID-W-035 run-wilhelm-de-jpg-literal-batch-hex9-13 · v1.0.0
 * Area: scripts/run-wilhelm-de-jpg-literal-batch-hex9-13.mjs
 * Family: FID-W
 *
 * Apply JPG literal corrections (hex 9–13), verify pilot, markHexComplete, report.
 */
...
async function main() {
  ...
  for (let hex = 9; hex <= 13; hex++) {
    const corrections = JPG_LITERAL_CORRECTIONS_9_13[hex] ?? {};
    ...
    if (verify.ok) {
      await markHexComplete(hex, {
        note: `JPG pp.${jpgPages} — AUD-DAT-W-07 literal hex 9-13`,
  ...
  const reportPath = join(REPORTS, `wilhelm-de-jpg-literal-audit-9-13-${stamp}.json`);
