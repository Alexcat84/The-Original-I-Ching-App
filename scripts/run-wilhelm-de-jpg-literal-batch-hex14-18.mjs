#!/usr/bin/env node
/**
 * QA code: AU-FID-W-036 run-wilhelm-de-jpg-literal-batch-hex14-18 · v1.0.0
 * Area: scripts/run-wilhelm-de-jpg-literal-batch-hex14-18.mjs
 * Family: FID-W
 *
 * Apply JPG literal corrections (hex 14–18), verify pilot, markHexComplete, report.
 */
...
async function main() {
  ...
  for (let hex = FROM; hex <= TO; hex++) {
    const corrections = JPG_LITERAL_CORRECTIONS_14_18[hex] ?? {};
    ...
    if (verify.ok) {
      await markHexComplete(hex, {
        note: `JPG pp.${jpgPages} — AUD-DAT-W-07 literal hex 14-18`,
  ...
  const reportPath = join(REPORTS, `wilhelm-de-jpg-literal-audit-14-18-${stamp}.json`);
