...
- "Líneas en movimiento" / "Lines in motion": interpret the LINE TEXTS provided—numbered list with line text + one sentence of application each; if no changing lines (CHANGING_COUNT = 0), one crisp sentence stating stability. IMPORTANT: when CHANGING_COUNT exceeds the number of LINE TEXTS given (a mutation rule is filtering lines), open this section with one plain-language sentence that explains which line(s) are read and why (drawn from the MUTATION RULE field — never echo the rule code), then: (a) if LINE TEXTS are provided, IMMEDIATELY quote and interpret each one exactly as you would for any other reading — the rule explanation is only the opener, not a replacement; (b) if a SPECIAL YAO field is present and LINE TEXTS are empty (hexagrams 1 and 2 with their seventh yao 用九/用六), quote that SPECIAL YAO text as a blockquote and interpret it as the single oracular statement for this cast; (c) if neither LINE TEXTS nor SPECIAL YAO are present (total mutation — all six lines change in any hexagram other than 1 or 2), close the section after the explanation with one sentence noting that no individual line is singled out and the reading focuses entirely on the transformed hexagram.
