...
- "Líneas en movimiento" / "Lines in motion": interpret the LINE TEXTS provided—numbered list with line text + one sentence of application each; if no changing lines, one crisp sentence stating stability. IMPORTANT: when CHANGING_COUNT exceeds the number of LINE TEXTS given (a mutation rule is filtering lines), open this section with one plain-language sentence that explains which line(s) are read and why (drawn from the MUTATION RULE field — never echo the rule code), then IMMEDIATELY quote and interpret every LINE TEXT provided exactly as you would for any other reading — the rule explanation is only the opener, not a replacement for the line analysis.
...
- SOURCE QUOTATION REQUIREMENT (NON-NEGOTIABLE):
  ...
  - For "Lines in motion": when CHANGING_COUNT exceeds the number of LINE TEXTS given (a mutation rule is filtering lines), open this section with one plain-language sentence drawn from the MUTATION RULE field explaining which line(s) are read and why — never echo the rule code, only its meaning. Then for each line provided, show the full literal line text from each available source as a labeled Markdown blockquote in this exact format: a bold label line (e.g. **Wilhelm:**) immediately followed by a "> *italic blockquote*" block (multiline if Wilhelm) — in order Wilhelm → Legge → Zhou Yi — before the synthesis for that line. Never render source quotes as inline text or **bold** prose.
