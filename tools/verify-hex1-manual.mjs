...
/** User-provided hex 1 transcription */
export const USER_HEX1 = {
  hex: "1",
  nombre: "The Creative",
  chinese: "乾",
  trigrama_arriba: "above CH'IEN THE CREATIVE, HEAVEN",
  trigrama_abajo: "below CH'IEN THE CREATIVE, HEAVEN",
  intro: `The first hexagram is made up of six unbroken lines. These unbroken lines stand for the primal power, which is lightgiving, active, strong, and of the spirit. The hexagram is consistently strong in character, and since it is without weakness, its essence is power or energy. Its image is heaven. Its energy is represented as unrestricted by any fixed conditions in space and is therefore conceived of as motion. Time is regarded as the basis of this motion. Thus the hexagram includes also the power of time and the power of persisting in time, that is, duration.
The power represented by the hexagram is to be interpreted in a dual sense—in terms of its action on the universe and of its action on the world of men. In relation to the universe, the hexagram expresses the strong, creative action of the Deity. In relation to the human world, it denotes the creative action of the holy man or sage, of the ruler or leader of men, who through his power awakens and develops their higher nature.`,
  judgment_oraculo: `THE CREATIVE works sublime success,
Furthering through perseverance.`,
  judgment_comentario: `According to the original meaning, the attributes [sublimity, potentiality of success, power to further, perseverance] are paired. When an individual draws this oracle, it means that success will come to him from the primal depths of the universe and that everything depends upon his seeking his happiness and that of others in one way only, that is, by perseverance in what is right.
...
Another line of speculation goes still further in separating the words "sublime," "success," "furthering," "perseverance," and parallels them with the four cardinal virtues in humanity. To sublimity, which, as the fundamental principle, embraces all the other attributes, it links love. To the attribute success are linked the mores, which regulate and organize the expressions of love and thereby make them successful. The attribute furthering is correlated with justice, which creates the conditions in which each receives that which accords with his being, that which is due him and which constitutes his happiness. The attribute perseverance is correlated with wisdom, which discerns the immutable laws of all that happens and can therefore bring about enduring conditions. These speculations, already broached in the commentary called Wên Yen,] later formed the bridge connecting the philosophy of the "five stages (elements) of change," as laid down in the Book of History (Shu Ching) with the philosophy of the Book of Changes, which is based solely on the polarity of positive and negative principles. In the course of time this combination of the two systems of thought opened the way for an increasingly intricate number symbolism.`,
  image_oraculo: `The movement of heaven is full of power.
Thus the superior man makes himself strong and untiring.`,
  image_comentario: `Since there is only one heaven, the doubling of the trigram Ch'ien, of which heaven is the image, indicates the movement of heaven. One complete revolution of heaven makes a day, and the repetition of the trigram means that each day is followed by another. This creates the idea of time. Since it is the same heaven moving with untiring power, there is also created the idea of duration both in and beyond time, a movement that never stops nor slackens, just as one day follows another in an unending course. This duration in time is the image of the power inherent in the Creative.
...
Here this creative force is still hidden beneath the earth and therefore has no effect. In terms of human affairs, this symbolizes a great man who is still unrecognized. Nonetheless he remains true to himself. He does not allow himself to be influenced by outward success or failure, but confident in his strength, he bides his time. Hence it is wise for the man who consults the oracle and draws this line to wait in the calm strength of patience. The time will fulfill itself. One need not fear lest strong will should not prevail; the main thing is not to expend one's powers prematurely in an attempt to obtain by force something for which the time is not yet ripe.`,
  L2_etiqueta: "Nine in the second place means:",
  L2_oraculo: `Dragon appearing in the field.
It furthers one to see the great man.`,
  ...
  L3_etiqueta: "Nine in the third place means:",
  L3_oraculo: `All day long the superior man is creatively active.
...
Danger. No blame.`,
  ...
  L4_etiqueta: "Nine in the fourth place means:",
  L4_oraculo: `Wavering flight over the depths.
No blame.`,
  L4_comentario:
  ...
  L5_oraculo: `Flying dragon in the heavens.
It furthers one to see the great man.`,
  L5_comentario:
  ...
  L6_etiqueta: "Nine at the top means:",
  L6_oraculo: "Arrogant dragon will have cause to repent.",
  L6_comentario:
  ...
  yong_etiqueta: "When all the lines are nines, it means:",
  yong_oraculo: `There appears a flight of dragons without heads.
Good fortune.`,
  yong_comentario:
...
function compareField(name, userVal, epubVal) {
  ...
  if (u.length === e.length && diffAt === 0 && u !== e) diffAt = minLen;
  return {
    name,
    status: "DIFF",
    userLen: u.length,
    epubLen: e.length,
    userSnippet: u.slice(Math.max(0, diffAt - 20), diffAt + 80),
    epubSnippet: e.slice(Math.max(0, diffAt - 20), diffAt + 80),
  };
}
...
for (const f of fields) {
  results.push(compareField(f, USER_HEX1[f], EPUB[f]));
}

console.log("=== USER vs EPUB FULL (hex 1) ===\n");
for (const r of results) {
  if (r.status === "OK") console.log(`OK  ${r.name}`);
  else if (r.status === "OK_NEWLINES_ONLY") console.log(`OK~ ${r.name} (solo diferencia de saltos)`);
  else {
...
console.log("\n=== USER ORACLE vs RUNTIME BUNDLE ===\n");
const oracleChecks = [
  ["judgment_oraculo", hex.judgment],
  ["image_oraculo", hex.image],
  ["L1_oraculo", hex.lines[0].text],
  ["L2_oraculo", hex.lines[1].text],
  ["L3_oraculo", hex.lines[2].text],
  ["L4_oraculo", hex.lines[3].text],
  ["L5_oraculo", hex.lines[4].text],
  ["L6_oraculo", hex.lines[5].text],
  ["yong_oraculo", hex.yongJiu],
];
for (const [name, runtime] of oracleChecks) {
  const r = compareField(name, USER_HEX1[name], runtime);
  console.log(`${r.status === "OK" || r.status === "OK_NEWLINES_ONLY" ? "OK" : "DIFF"} ${name}`);
  if (r.status === "DIFF") {
    console.log(`  user: ${JSON.stringify(normalizeText(USER_HEX1[name]))}`);
    console.log(`  runtime: ${JSON.stringify(normalizeText(runtime))}`);
  }
}

// L4 check - user message might omit "No blame."
const l4epub = EPUB.L4_oraculo;
console.log("\n=== L4_oraculo EPUB exact ===");
console.log(JSON.stringify(l4epub));

// judgment typo Wên Yen
console.log("\n=== judgment_comentario EPUB around Wên Yen ===");
const jc = EPUB.judgment_comentario;
const idx = jc.indexOf("Wên Yen");
console.log(JSON.stringify(jc.slice(idx - 20, idx + 40)));
}
