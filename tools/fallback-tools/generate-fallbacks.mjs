...
// black-forest-labs/FLUX.1-schnell left Together's serverless platform on
// 2026-08-19. Default follows the app (see image-provider.ts); override with
// TOGETHER_IMAGE_MODEL to regenerate the pool with a different model.
const MODEL = process.env.TOGETHER_IMAGE_MODEL?.trim() || "Rundiffusion/Juggernaut-Lightning-Flux";
// schnell capped at 4-12 steps; the Juggernaut/FLUX-dev family takes 20.
const STEPS = Number(process.env.TOGETHER_IMAGE_STEPS ?? (/schnell/i.test(MODEL) ? 4 : 20));
const CONCURRENCY = 1;       // Together rate limit: 50 RPM — 1 worker keeps us safe
const DELAY_MS = Number(process.env.TOGETHER_DELAY_MS ?? 1500);
const MAX_RETRIES = 5;       // longer retry chain for rate-limit spikes
const MAX_STAMP_RETRIES = 2;
// Variant range, inclusive. Growing the pool means generating a NEW range (e.g.
// VARIANT_FROM=11 VARIANT_TO=20) and uploading it BEFORE raising
// R2_VARIANTS_PER_KEY in the app, or the picker points at objects that do not
// exist yet.
const VARIANT_FROM = Number(process.env.VARIANT_FROM ?? 1);
const VARIANT_TO = Number(process.env.VARIANT_TO ?? 10);
const VARIANTS = VARIANT_TO;
...
// Cost estimate only (printed to the operator). schnell billed per megapixel at
// $0.0027/MP; the Juggernaut family bills a FLAT rate per image regardless of
// size, which is why the pricier tiers got cheaper with the migration.
const PER_MEGAPIXEL = /schnell/i.test(MODEL) ? 0.0027 : 0;
const FLAT_PER_IMAGE = /juggernaut-lightning/i.test(MODEL)
  ? 0.0017
  : /juggernaut-pro/i.test(MODEL)
    ? 0.0049
    : 0.003;
const px = (w, h) => (PER_MEGAPIXEL ? (w * h) / 1_000_000 * PER_MEGAPIXEL : FLAT_PER_IMAGE);
const COST = {
  free:         px(1024, 768),
  seeker:       px(1024, 1024),
  practitioner: px(1184, 1184),
  master:       px(1504, 1504),
};
...
const BONES_STATES = [
  ...
  // NOTE: the "silence" verdict was removed from the oracle-bones engine (its
  // 15% weight was redistributed across the four remaining verdicts, see
  // packages/oracle-bones-engine/src/engine.ts). OracleBonesVerdict now has
  // exactly these four members, and image-provider.ts maps only these four to
  // R2 folders, so generating a silence pool would produce images nothing can
  // ever serve. Do not re-add it without a verdict to go with it.
];
...
function buildPrompt(subject, variant) {
  ...
  // `^` yields a SIGNED int32 and this is the only axis mixing the raw `h`, so
  // without the `>>> 0` the index went negative and OPENER_VARIANTS[-n] was
  // undefined, which the trailing .filter(Boolean) then dropped in silence:
  // 46.6% of images were generated with no opening line at all. Same defect and
  // same fix as packages/image-engine/src/prompt.ts (this file keeps its own
  // copy of the rotation logic).
...
/**
 * Nearest valid request size for the serving stack, rounding UP.
 * Juggernaut/FLUX reject anything that is not a multiple of 64 between 128 and
 * 2048, and two of our tier sizes are not (practitioner 1184 = 18.5x64, master
 * 1504 = 23.5x64). The FILE must still be exactly the tier size, because the R2
 * key embeds it (`.../1184x1184.webp`) and the picker builds that path blindly,
 * so we ask for the next valid size up and scale back down: downscaling keeps
 * detail, upscaling would invent it.
 */
const MULT = 64;
function requestSize(v) {
  return Math.min(2048, Math.max(128, Math.ceil(v / MULT) * MULT));
}

async function generateImage({ prompt, width, height }) {
  const reqW = requestSize(width);
  const reqH = requestSize(height);
  for (let attempt = 0; attempt <= MAX_RETRIES; attempt++) {
    ...
    // Scale back to the exact tier size when the request had to be rounded up,
    // so the stored object matches the key the picker will ask for.
...
function buildAllJobs() {
  ...
  for (const hex of HEXAGRAMS) {
    for (let v = VARIANT_FROM; v <= VARIANT_TO; v++) {
...
function buildTestJobs() {
  return [
    ...
    // Was bones_silence (BONES_STATES[4]); that verdict no longer exists, so the
    // index would now be undefined. Cover the opposite pole instead.
    { key: "TEST/bones_xiong_clear_master", subject: BONES_STATES[3], v: 1, tier: TIERS[3], filename: "bones_xiong_clear_master.webp" },
