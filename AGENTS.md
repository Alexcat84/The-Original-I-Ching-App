...
- i18n centralizado en `@iching-oracle/i18n` (`packages/i18n`) con `Record<AppLocale, …>` por módulo; referencia EN (`DEFAULT_LOCALE` en `locales.ts`); no usa `next-intl` ni `apps/web/messages/*.json`. Al añadir idioma, seguir [`docs/workflows/I18N_GUIDE.md`](docs/workflows/I18N_GUIDE.md) (`page.tsx`, mobile, backend Claude, SEO).
