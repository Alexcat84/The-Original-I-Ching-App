...
│   └── db/migrations/          # migraciones SQL (69 numeradas hasta 076; replayable en DB vacía)
...
- [x] EAS Build configurado. **INVARIANTE (2026-07-15): APK de smoke/staging: SIEMPRE build local (`assembleRelease`) con el `.env` volteado a PREVIEW/staging. EAS: solo AAB de producción (release).** El `.env` de mobile es auto-documentado (`# PREVIEW = staging | PRODUCTION = comentado`): tiene pares staging(comentado)/producción(activo) para `EXPO_PUBLIC_SUPABASE_URL`+`ANON_KEY`, `EXPO_PUBLIC_API_URL` y `APP_ENV`/`MOBILE_API_MODE`. Para smoke se voltean **TODOS los pares** a staging (no basta quitar la API_URL: sin el par de Supabase de staging el APK crashea al abrir, `index.tsx` lanza throw si faltan, y con el de producción el PKCE nativo↔web se rompe por proyectos Supabase distintos). El perfil `preview` de EAS NO fija estas variables y su dashboard apunta a producción: un APK de EAS preview sale con la URL de PRODUCCIÓN embebida y no sirve para smoke (riesgo de compras/consultas reales pre-merge). Ver runbook RUN-SUP-03 (instala desde `android/app/build/outputs/apk/release/`).
...
### Migraciones DB (69 numeradas, la más alta 076; selección de hitos)
